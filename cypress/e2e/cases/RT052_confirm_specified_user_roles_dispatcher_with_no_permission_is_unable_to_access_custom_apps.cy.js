describe('RT052: Confirm specified user roles dispatcher with no permission is unable to access custom apps', () => {
    // Define shared data.
    const shared_data = {
    ...
    // The administrator navigates to the app settings page, selects a coded app, and verifies that the dispatcher role is checked.
    it('The administrator navigates to the app settings page, selects a coded app, and verifies that the dispatcher role is unchecked.', () => {
        cy.session(
            'the_administrator_navigates_to_the_app_settings_page_selects_a_coded_app_and_verifies_that_the_dispatcher_role_is_unchecked',
            () => {
                ...
                // Administrator confirms dispatcher role is checked and updates settings.
                cy.get(
                    'input.apps-user-role[name="roles[]"][value="dispatcher"]'
                ).uncheck()
    ...
    it('Administrator navigates to D.T Roles Settings and confirms access to coded app permission is selected.', () => {
        cy.session(
            ...
            () => {
                ...
                // cy.get(`input[value="can_access_home_screen"]`).should(
                //     'checked'
                // )
    ...
    it('The administrator goes to the Users page, locates the new test user  with the dispatcher role, and sets the password.', () => {
        cy.session(
            ...
            () => {
                ...
                //unchecked the all the roles
                cy.get('input[name="dt_multi_role_user_roles[]"]').uncheck()

                // check the dispatcher role
                cy.get(
    ...
    it('Log in to the home screen and verify that the configured coded app is not visible and accessible.', () => {
        cy.session(
            'log_in_to_the_home_screen_and_verify_that_the_configured_coded_app_is_not_visible_and_accessible',
            () => {
                ...
                // Confirm configured app is not visible within frontend home screen.
