jQuery(document).ready(function ($) {
                    ...
                    $('#dialog_icon_selector_icons_search_spinner')
                        ...
                        .fadeOut('fast', function () {
                            ...
                            if (filtered_icons.length > 0) {
                                $('#dialog_icon_selector_icons_div').fadeIn(
                                    'fast'
                                )
                            } else {
    ...
    $('.save-drag-changes').on('click', function () {
        ...
        $('.sort-tile').each((a, b) => {
            ...
            $(`#${tile_key} .connectedSortable li`).each(
                (field_index, field) => {
    ...
    $('#connection_field_target').on('change', function () {
        let post_type_label = $(
            '#connection_field_target option:selected'
        ).text()
                ...
                .done(function (response) {
                    ...
                    $('#custom_description_translation_count').html(
                        response['description_translations']
                            ? Object.keys(response['description_translations'])
                                  .length
                            : 0
                    )
                    if (
                        ...
                        $('.sortable-field-options')
                            ...
                            .each(function (idx, tr) {
                                ...
                                $(tr)
                                    .find(
                                        '#option_description_translation_count'
                                    )
                                    .html(
                                        response['defaults'] &&
                                            response['defaults'][option_key] &&
                ...
                if (option_key) {
                    ...
                    $(tr)
                        ...
                        .each(function (okt_idx, okt_input) {
                            ...
                            let locale = okt_input.id.replace(
                                option_key_prefix,
                                ''
                            )
                    ...
                    $(tr)
                        .find(
                            "input[id^='" + option_key_description_prefix + "']"
                        )
